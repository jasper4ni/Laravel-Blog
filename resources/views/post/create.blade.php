...
<div class="bg-gray-700 font-mono">
    ...
    @endif
    <form class="rounded-md sm:w-10/12 sm:px-0 px-4 py-10 m-auto" action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div>
            <div>
                <x-label class="text-2xl font-bold" :value="__('New Post')"/>
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" class='rounded-md mt-2 shadow-sm w-full h-24 h- outline-none border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50'></textarea>
            </div>
            ...
            <div class="w-full cursor-pointer mb-5 text-white h-auto border-2 flex sm:flex-row flex-col relative bg-black bg-opacity-40 border-gray-500 rounded mx-auto px-5 pt-5 sm:pb-12 pb-14 hover:bg-opacity-50 transition ease-in-out duration-150">
                <div id="image_overlay" class="bg-black bg-opacity-20 relative">
                    ...
                    <img src="" alt="Image" id="img_preview" class="h-40 sm:max-w-none sm:w-72 w-full opacity-0 transition ease-in-out duration-500" style="object-fit: cover">
                </div>
